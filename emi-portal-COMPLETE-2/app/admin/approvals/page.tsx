        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-ink-muted text-sm mt-1">
              Review and approve retailer payment requests
            </p>
          </div>
            ...
            <div className="flex bg-surface-2 rounded-xl p-1 border border-surface-4">
              <button
                onClick={() => {
                  ...
                  fetchPending(searchQuery || undefined, 'PENDING');
                }}
                className={`px-4 py-1.5 rounded-lg text-xs font-medium transition-all ${
                  statusFilter === 'PENDING'
                    ? 'bg-brand-500 text-ink shadow'
                    : 'text-ink-muted'
                }`}
              >
                Pending
              ...
              <button
                onClick={() => {
                  setStatusFilter('ALL');
                  fetchPending(searchQuery || undefined, 'ALL');
                }}
                className={`px-4 py-1.5 rounded-lg text-xs font-medium transition-all ${
                  statusFilter === 'ALL' ? 'bg-brand-500 text-ink shadow' : 'text-ink-muted'
                }`}
              >
                All Payments
              </button>
        ...
        {requests === null && !loading && (
          <div className="flex flex-col items-center justify-center py-20 text-center animate-fade-in">
            ...
            <p className="text-ink-muted text-lg">
              Search for pending requests or click "Load All Pending"
            </p>
          </div>
        )}
        ...
        {requests !== null && requests.length > 0 && (
          <div className="space-y-4">
            {requests.map((req) => {
              ...
              const retailer = req.retailer as {
                name?: string;
                username?: string;
              };
              ...
              return (
                <div
                  key={req.id}
                  className={`card p-5 animate-fade-in transition-opacity ${
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
                    ...
                    {hasFirstCharge && (
                      <div className="bg-brand-50 border border-brand-200 rounded-xl p-3">
                        ...
                        <p className="num font-semibold text-brand-600">
                          {fmt(req.first_emi_charge_amount)}
                        </p>
                      </div>
                    )}
      ...
      {rejectModal && (
        ...
        >
          <div className="card w-full max-w-md p-6 animate-slide-up shadow-modal">
            <h3 className="font-display text-xl font-bold text-danger mb-1">
        ...
        >
          <div className="card w-full max-w-lg p-6 animate-slide-up shadow-modal max-h-[90vh] overflow-y-auto">
            ...
            <p className="text-sm text-ink-muted mb-5">
              #{editModal.id.slice(0, 8).toUpperCase()} —{' '}
              {((editModal.customer as Record<string, unknown>)?.customer_name as string) || 'Customer'}
            </p>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="number"
                    value={editForm.total_emi_amount}
                    onChange={(e) =>
                      setEditForm((f) => ({ ...f, total_emi_amount: e.target.value }))
                    }
                    className="input"
                  />
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="number"
                    value={editForm.first_emi_charge_amount}
                    onChange={(e) =>
                      setEditForm((f) => ({
                        ...f,
                        first_emi_charge_amount: e.target.value,
                      }))
                    }
                    className="input"
                  />
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    value={editForm.collected_by_role}
                    onChange={(e) =>
                      setEditForm((f) => ({ ...f, collected_by_role: e.target.value }))
                    }
                    className="input"
                  >
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={() => setEditModal(null)} className="btn-ghost flex-1">
                Cancel
              </button>
